...
The self-supervised configuration schedules SimCLR training for 200 epochs.The completed metrics: linear evaluation accuracy reached 67.05\%, and the selected test-time evaluation improved from 48.34\% without adaptation to 55.78\% after ActMAD TTT. The gain is 7.44 percentage points.
...
The self-supervised configuration is \texttt{configs/self\_supervised.yaml}. It uses CIFAR-10 at \texttt{data/raw}, enables download, and sets the training method to SimCLR. The current configuration uses 200 epochs. The recorded metrics in this report still come from the earlier 100-epoch terminal output.
...
The available numbers are summarized in Table~\ref{tab:final-summary}. The current configuration now uses 200 SSL epochs, while the saved metric values come from the earlier 100-epoch SSL run and the selected TTT evaluation.
...
Recorded self-supervised run epochs & 100\\
...
\begin{table}[H]
\centering
\caption{Selected TTT evaluation.}
...
These results cover the selected setting from \texttt{configs/test\_time\_training.yaml}. They are not a full CIFAR-10-C benchmark: the final files do not include a CSV over all 15 corruptions or a mean score over all corruptions.

\subsection{Training loss evolution}

The saved SimCLR loss log decreased from 5.6214 at epoch 1 to 4.6586 at epoch 100. The curve in Figure~\ref{fig:training-loss} is built only from the 100 losses printed in the earlier terminal log. After rerunning the 200-epoch configuration, this figure and table should be regenerated from the new log.
...
The recorded results are positive for the selected setting. The TTT evaluation improved accuracy from 48.34\% to 55.78\%, a gain of 7.44 percentage points. 
