...
The final run gives a useful result on the selected corrupted evaluation. SimCLR training completed for 100 epochs, and the linear evaluation accuracy reached 67.05\%. The test-time evaluation improved from 48.34\% without adaptation to 55.78\% after ActMAD TTT. The gain is 7.44 percentage points.
...
The report separates three cases. The first is clean CIFAR-10 source training and linear evaluation. The second is the selected test-time evaluation on corrupted CIFAR-10 data. The third is the larger all-corruption CIFAR-10-C evaluation.
...
The self-supervised configuration is \texttt{configs/self\_supervised.yaml}. It uses CIFAR-10 at \texttt{data/raw}, enables download, and sets the training method to SimCLR. The values used in the report come from the config file and the final terminal output.
...
Dataset root in config & \texttt{data/raw}\\
Resolved CIFAR-10-C directory & \texttt{data/raw/CIFAR-10-C/}\\
...
results/self_supervised/source_stats.pt
\end{lstlisting}
These files are produced by the self-supervised stage. The TTT stage loads them. A missing \texttt{.pt} file inside \texttt{results/test\_time\_training/} is not automatically an error, because the TTT stage evaluates and adapts the model rather than saving a new checkpoint.
...
\begin{lstlisting}
data/raw/CIFAR-10-C/shot_noise.npy
\end{lstlisting}
The labels are expected in:
\begin{lstlisting}
data/raw/CIFAR-10-C/labels.npy
\end{lstlisting}
...
The final successful TTT command used the same \texttt{configs/test\_time\_training.yaml} path after the dataset issue had been corrected.
...
The final TTT command completed after the dataset path issue had been corrected. It loaded the saved backbone and classifier, evaluated the baseline model, then applied ActMAD TTT. The baseline accuracy was 48.34\%, the ActMAD TTT accuracy was 55.78\%, and the improvement was 7.44 percentage points.
...
The final numbers are summarized in Table~\ref{tab:final-summary}. They come from the terminal output of the completed SSL run and the TTT evaluation.

\begin{table}[H]
\centering
\caption{Final numerical results.}
...
The implemented pipeline is coherent. The SSL stage produces a usable backbone, the linear classifier checks the learned features, source statistics are saved, and TTT loads the saved files and performs adaptation.
...
The final results are positive for the selected setting. The self-supervised run completed 100 epochs and reached 67.05\% linear evaluation accuracy. The TTT evaluation improved accuracy from 48.34\% to 55.78\%, a gain of 7.44 percentage points. This supports the usefulness of activation matching in the evaluated setting.
