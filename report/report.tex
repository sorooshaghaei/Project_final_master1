...
The self-supervised configuration schedules SimCLR training for 200 epochs. In the training log, the linear evaluation accuracy reached 67.05\%. The final saved TTT result is reported on CIFAR-10-C \texttt{shot\_noise} at severity 5: accuracy increased from 33.02\% without adaptation to 69.12\% after ActMAD TTT, a gain of 36.10 percentage points. 
...
\end{lstlisting}
The loss was printed once per epoch. The initial loss was 5.9092 and the final recorded loss at epoch 200 was ???????.

\subsection{Linear evaluation}

After SimCLR pretraining, \texttt{run.py} calls the linear evaluation step. The backbone is frozen and a linear classifier is trained on top of the learned features. The classifier is then evaluated on CIFAR-10. The final linear evaluation accuracy was ?????.
...
This command writes the selected TTT result to \texttt{results/test\_time\_training/results.csv}. The CSV stores the corruption, severity, baseline accuracy, ActMAD accuracy, improvement, and the number of correct predictions used in the results section.
...
The numbers are summarized in Table~\ref{tab:final-summary}. The SSL values come from the recorded training log. The TTT values come from \texttt{results/test\_time\_training/results.csv}.
...
TTT corruption and severity & \texttt{shot\_noise}, severity 5\\
Baseline accuracy & 33.02\%\\
ActMAD TTT accuracy & 69.12\%\\
Improvement & +36.10 percentage points\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lcccc}
\toprule
Evaluation setting & Baseline & ActMAD TTT & Improvement & Correct predictions\\
\midrule
\texttt{shot\_noise}, severity 5 & 33.02\% & 69.12\% & +36.10 pp & 3302 to 6912 / 10000\\
\bottomrule
\end{tabular}
\end{table}

These results cover the selected setting from \texttt{configs/test\_time\_training.yaml}. 

\subsection{Training loss evolution}

The saved SimCLR loss log decreased from 5.6214 at epoch 1 to 4.6586 at epoch 100. The curve in Figure~\ref{fig:training-loss} is built from the 100 losses printed in the available terminal log. A complete 200-epoch ????????
...
The final selected evaluation gives 33.02\% accuracy without adaptation and 69.12\% after ActMAD TTT. The gain is 36.10 percentage points. This is the clearest result of the project: for severe \texttt{shot\_noise}, the activation matching step changed many wrong baseline predictions into correct ones.

The baseline accuracy on this corrupted setting is much lower than the 67.05\% linear evaluation accuracy on clean CIFAR-10. This is consistent with the distribution-shift problem. The class set is unchanged, but the image statistics are different enough that the frozen encoder and classifier lose reliability.

ActMAD recovers more than the clean linear-evaluation gap in this selected setting. One possible explanation is that severe noise strongly perturbs normalization-layer activations, so matching target statistics back toward the source statistics is especially useful. However, this interpretation should remain cautious. The result shows that the method worked well here; it does not prove that the same gain will appear for blur, weather, compression, or lower severity levels.
...
The SSL stage produces a usable backbone, the linear classifier checks the learned features, source statistics are saved, and TTT loads the saved files and performs adaptation.

% =================================================
\section{Limitations}
% =================================================

The main limitation is evaluation coverage. The final TTT CSV reports one selected setting: \texttt{shot\_noise} at severity 5. It does not yet include a table over all 15 CIFAR-10-C corruptions or a mean corruption score. This means the project can report a strong selected result, but it cannot claim general robustness across CIFAR-10-C.
...
data/raw/cifar10c/CIFAR-10-C/
\end{lstlisting}
The all-corruption script can be used for a wider evaluation. Its output should be saved before adding claims about average robustness.
...
The recorded result is positive for the selected setting: ActMAD TTT strongly improved performance on severe \texttt{shot\_noise}. The main conclusion is therefore not that the method is universally robust, but that activation matching can be useful when the tested corruption strongly changes internal feature statistics.
