...
The final run gives a useful result on the configured corrupted evaluation. SimCLR training completed for 100 epochs, and the linear evaluation accuracy reached 67.05\%. The test-time evaluation improved from 48.34\% without adaptation to 55.78\% after ActMAD TTT. The gain is 7.44 percentage points. This is the confirmed result for the final evaluated setting, not a full benchmark over all CIFAR-10-C corruptions.
...
CIFAR-10-C was introduced as part of the common corruptions benchmark proposed by Hendrycks and Dietterich \citep{hendrycks2019robustness}. The benchmark is useful because it avoids vague robustness claims. A method can be tested on fixed corruptions and severity levels. If the full benchmark is used, accuracy or error can be reported per corruption and then averaged. In this project, the final run reports the configured corrupted evaluation from \texttt{configs/test\_time\_training.yaml}, not the complete 15-corruption CIFAR-10-C benchmark.
...
The report separates three cases. The first is clean CIFAR-10 source training and linear evaluation. The second is the configured test-time evaluation on corrupted CIFAR-10 data. The third is the larger all-corruption CIFAR-10-C evaluation. The final terminal output confirms the source-domain training result and the configured TTT result. It does not provide a table over all 15 CIFAR-10-C corruptions.
...
The final successful TTT command used the same \texttt{configs/test\_time\_training.yaml} path after the dataset issue had been corrected. The reported numbers are therefore treated as the final configured test-time evaluation. They are still not a complete CIFAR-10-C benchmark because the provided results do not include the full 15-corruption table.
...
The final TTT command completed after the dataset path issue had been corrected. It loaded the saved backbone and classifier, evaluated the baseline model, then applied ActMAD TTT. The baseline accuracy was 48.34\%, the ActMAD TTT accuracy was 55.78\%, and the improvement was 7.44 percentage points. No per-corruption CSV was provided with that output.
...
\begin{table}[H]
\centering
\caption{Configured TTT evaluation.}
\label{tab:available-ttt}
\begin{tabular}{lccc}
\toprule
Evaluation setting & Baseline accuracy & ActMAD TTT accuracy & Improvement\\
\midrule
Configured TTT evaluation & 48.34\% & 55.78\% & +7.44 pp\\
\bottomrule
\end{tabular}
\end{table}

The table should not be read as a full CIFAR-10-C benchmark over all corruption types. It is the final configured TTT evaluation: the trained backbone, classifier, and source statistics are loaded, then the baseline and adapted predictions are compared on the evaluated corrupted setting.
...
\subsection{CIFAR-10-C evaluation coverage}

The final TTT run completed for the configured evaluation setting and produced a baseline/adapted comparison. The provided results do not include a full all-corruption CSV. Table~\ref{tab:cifar10c-status} separates the confirmed final result from the benchmark elements that were not provided.
...
Configured TTT run & Confirmed by terminal output after the dataset path issue was fixed.\\
...
The final configured evaluation gives 48.34\% accuracy without adaptation and 55.78\% after ActMAD TTT. The gain is 7.44 percentage points. This result supports the idea that activation matching can help under the evaluated shift, but it is not enough to claim full robustness on CIFAR-10-C.
...
Third, the result covers one configured evaluation setting, not the full CIFAR-10-C benchmark. The final run supports the method on the evaluated setting, but it does not show behavior across all corruption families. The missing element is experimental coverage, not the validity of the final baseline/TTT comparison.
...
The main positive point is that the implemented pipeline is coherent. The SSL stage produces a usable backbone, the linear classifier gives a representation-quality check, source statistics are saved, and TTT loads the saved files and performs adaptation. The main weak point is experimental coverage. The available result is a useful final run for the TER report, but a stronger scientific conclusion would require a full per-corruption evaluation over all CIFAR-10-C corruptions.

% =================================================
\section{Limitations}
% =================================================

The first limitation is evaluation coverage. The final TTT run gives a clear result for the configured corrupted evaluation, but the provided results do not include a full table over all 15 CIFAR-10-C corruptions.
...
The method should also be validated with the full all-corruption script. That would produce per-corruption results and mean accuracy values rather than a single configured TTT result.
...
data/raw/cifar10c/CIFAR-10-C/
\end{lstlisting}
The final single-setting TTT result is already reported above. A full benchmark would require running the all-corruption script and saving the resulting table.
...
The final results are positive but limited in coverage. The self-supervised run completed 100 epochs and reached 67.05\% linear evaluation accuracy. The configured TTT evaluation improved accuracy from 48.34\% to 55.78\%, a gain of 7.44 percentage points. This supports the usefulness of activation matching in the evaluated setting.

The result is not a complete robustness benchmark. No per-corruption CSV was provided. A stronger final experiment would evaluate all 15 CIFAR-10-C corruptions, report mean accuracy, and compare several adaptation settings.

The next practical step is to run \texttt{scripts/eval\_corruption.py}, save the resulting CSV, and compare ActMAD across all listed corruptions and severity levels. It would also be useful to test several adaptation step counts, several batch sizes, and at least one other test-time adaptation method. Safer adaptation criteria should also be studied, because an unsupervised adaptation loss can improve activation matching while hurting classification.
